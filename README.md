...
publish-sign (canonical Distribution robot + OIDC; never executes product source)
            |
            v
promotion (release evidence + read-only registry verification + OIDC;
           no package/source/publication credential)
            |
            v
...
- `ARCONATH_REGISTRY_USERNAME` and `ARCONATH_REGISTRY_PASSWORD` secrets on the
  ...
  is absent.
- `ARCONATH_REGISTRY_READ_USERNAME` and `ARCONATH_REGISTRY_READ_PASSWORD`
  secrets on the protected `promotion` environment for a read-only verifier.
  They are used only to inspect the exact non-zero rollback digest; the
  promotion job never receives publication credentials or registry write access.
...
reader and registry credentials. Registry publication robot credentials stay
only in the publication environment and are never exposed to source-fetch,
build-test, or promotion. Promotion has a separate read-only registry verifier
credential for rollback admission and removes it after the job.
...
The promotion workflow adds a live cryptographic admission gate immediately
before manifest emission: it re-verifies the downloaded artifact and every
attestation against the exact digest and local predicate, then verifies any
non-zero rollback digest in the canonical registry with an exact-workflow
signature and provenance proof. A zero rollback digest is the explicit
