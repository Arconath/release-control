...
COSIGN_WORKFLOW_IDENTITY = (
    "https://github.com/Arconath/release-control/"
    ".github/workflows/release.yml@refs/heads/main"
)
COSIGN_WORKFLOW_IDENTITY_REGEXP = (
    r"^https://github\.com/Arconath/release-control/\.github/workflows/"
    r"release\.yml@refs/heads/main$"
)
COSIGN_OIDC_ISSUER = "https://token.actions.githubusercontent.com"
COSIGN_WORKFLOW_TRIGGER = "workflow_dispatch"
COSIGN_WORKFLOW_NAME = "Trusted product release"
COSIGN_WORKFLOW_REPOSITORY = "Arconath/release-control"
COSIGN_WORKFLOW_REF = "refs/heads/main"
COSIGN_PROVENANCE_TYPE = "https://slsa.dev/provenance/v1"
COSIGN_ATTESTATION_SPECS = (
    (
...
def _load_verified_attestation_statement(
    envelope_path: Path,
    predicate_type: str,
    artifact_repository: str,
    artifact_digest: str,
) -> dict[str, Any]:
    """Decode a Cosign-verified DSSE envelope and bind its subject identity."""
...
def verify_attestation_payload(
    ...
    statement = _load_verified_attestation_statement(
        envelope_path,
        predicate_type,
        artifact_repository,
        artifact_digest,
...
def _validate_rollback_provenance_value(
    value: dict[str, Any],
    intent: dict[str, Any],
    artifact_repository: str,
    artifact_digest: str,
) -> str:
    """Validate previous-release provenance independently of the new intent."""

    strict_keys(value, {"buildDefinition", "runDetails"}, "rollback provenance predicate")
    repository = validate_artifact_repository(
        artifact_repository, "rollback provenance artifact repository"
    )
    digest = require_nonzero_digest(
        artifact_digest, "rollback provenance artifact digest"
    )
    intent_source = intent.get("source")
    if not isinstance(intent_source, dict):
        die("rollback provenance intent source must be an object")
    strict_keys(intent_source, {"repository", "commit_sha", "tree_sha"}, "rollback provenance intent source")
    expected_source_repository = require_string(
        intent_source["repository"], SOURCE_REPO, "rollback provenance intent source.repository"
    )

    build_definition = value["buildDefinition"]
    if not isinstance(build_definition, dict):
        die("rollback provenance buildDefinition must be an object")
    strict_keys(
        build_definition,
        {"buildType", "externalParameters", "internalParameters", "resolvedDependencies"},
        "rollback provenance buildDefinition",
    )
    if build_definition["buildType"] != "https://arconath.com/release-control/build/v1":
        die("rollback provenance build type is not canonical")

    external_parameters = build_definition["externalParameters"]
    if not isinstance(external_parameters, dict):
        die("rollback provenance externalParameters must be an object")
    strict_keys(
        external_parameters,
        {"source", "artifact"},
        "rollback provenance externalParameters",
    )
    source = external_parameters["source"]
    if not isinstance(source, dict):
        die("rollback provenance source must be an object")
    strict_keys(source, {"repository", "commit_sha", "tree_sha"}, "rollback provenance source")
    source_repository = require_string(
        source["repository"], SOURCE_REPO, "rollback provenance source.repository"
    )
    if source_repository != expected_source_repository:
        die("rollback provenance source repository does not match the current artifact intent")
    source_commit = require_string(
        source["commit_sha"], SHA, "rollback provenance source.commit_sha"
    )
    source_tree = require_string(
        source["tree_sha"], SHA, "rollback provenance source.tree_sha"
    )

    artifact = external_parameters["artifact"]
    if not isinstance(artifact, dict):
        die("rollback provenance artifact must be an object")
    strict_keys(
        artifact,
        {"repository", "version", "digest"},
        "rollback provenance artifact",
    )
    if artifact["repository"] != repository:
        die("rollback provenance artifact repository identity does not match")
    require_string(artifact["version"], VERSION, "rollback provenance artifact version")
    if artifact["digest"] != digest:
        die("rollback provenance artifact digest does not match")

    internal_parameters = build_definition["internalParameters"]
    if not isinstance(internal_parameters, dict):
        die("rollback provenance internalParameters must be an object")
    strict_keys(
        internal_parameters,
        {"release_control_sha"},
        "rollback provenance internalParameters",
    )
    control_sha = require_string(
        internal_parameters["release_control_sha"],
        GIT_SHA,
        "rollback provenance release-control SHA",
    )
    ...
    if (
        ...
        die("rollback provenance must contain exactly one resolved source dependency")
    dependency = dependencies[0]
    strict_keys(dependency, {"uri", "digest"}, "rollback provenance resolved dependency")
    if dependency["uri"] != f"git+https://github.com/{source_repository}":
        die("rollback provenance source URI does not match source repository")
    dependency_digest = dependency["digest"]
    if not isinstance(dependency_digest, dict):
        die("rollback provenance source digest must be an object")
    strict_keys(
        dependency_digest,
        {"gitCommit", "gitTree"},
        "rollback provenance source digest",
    )
    if dependency_digest != {"gitCommit": source_commit, "gitTree": source_tree}:
        die("rollback provenance source digest does not match source identity")

    run_details = value["runDetails"]
    if not isinstance(run_details, dict):
        die("rollback provenance runDetails must be an object")
    strict_keys(run_details, {"builder", "metadata"}, "rollback provenance runDetails")
    builder = run_details["builder"]
    if not isinstance(builder, dict):
        die("rollback provenance builder must be an object")
    strict_keys(builder, {"id"}, "rollback provenance builder")
    if builder["id"] != COSIGN_WORKFLOW_IDENTITY:
        die("rollback provenance builder workflow is not canonical")
    metadata = run_details["metadata"]
    if not isinstance(metadata, dict):
        die("rollback provenance metadata must be an object")
    strict_keys(metadata, {"invocationId"}, "rollback provenance metadata")
    invocation_id = require_string(
        metadata["invocationId"],
        re.compile(r"^[a-z0-9][a-z0-9._-]{7,127}@[0-9a-f]{40}$"),
        "rollback provenance invocation ID",
    )
    if invocation_id.rsplit("@", 1)[1] != control_sha:
        die("rollback provenance invocation ID does not match release-control SHA")
    return control_sha
...
def _run_external_command(
    ...
    *,
    ...
    """Run a verification tool without exposing command output or credentials."""
    ...
    try:
        ...
        die(f"{context} requires {command[0]}")
    except subprocess.TimeoutExpired:
        die(f"{context} timed out")
    except OSError:
        die(f"{context} could not be executed")
    if result.returncode:
        die(f"{context} failed (exit {result.returncode})")
    if stdout is not None:
        return b""
    output = result.stdout
    if not isinstance(output, (bytes, bytearray)):
        die(f"{context} returned non-byte output")
    return bytes(output)


def _cosign_workflow_claims(workflow_sha: str | None = None) -> list[str]:
    claims = [
        ...
        COSIGN_WORKFLOW_TRIGGER,
        "--certificate-github-workflow-name",
        COSIGN_WORKFLOW_NAME,
        "--certificate-github-workflow-repository",
        COSIGN_WORKFLOW_REPOSITORY,
    ...
    if workflow_sha is not None:
        claims.extend(
            [
                ...
                require_string(workflow_sha, GIT_SHA, "Cosign workflow SHA"),
            ]
        )
    return claims


def verify_rollback_digest(intent: dict[str, Any]) -> dict[str, Any]:
    """Verify the previous release in the canonical registry before rollback emission."""

    artifact = intent.get("artifact")
    if not isinstance(artifact, dict):
        die("rollback artifact must be an object")
    strict_keys(artifact, {"repository", "version"}, "rollback intent artifact")
    repository = validate_artifact_repository(
        artifact["repository"], "rollback artifact repository"
    )
    require_string(artifact["version"], VERSION, "rollback artifact version")
    if not repository.startswith(CANONICAL_ARTIFACT_PREFIX):
        die("rollback artifact repository is outside the canonical registry namespace")
    rollback = intent.get("rollback")
    if not isinstance(rollback, dict):
        die("rollback intent must be an object")
    strict_keys(rollback, {"previous_digest", "reason"}, "rollback intent")
    previous_digest = require_string(
        rollback["previous_digest"], DIGEST, "rollback previous digest"
    )
    reference = f"{repository}@{previous_digest}"
    if previous_digest == ZERO_DIGEST:
        return {
            "status": "baseline",
            "verification": "not-required",
            "reason": "no previous release exists for the first release",
            "artifact": {
    ...
    registry_reference = f"docker://{reference}"
    inspect_output = _run_external_command(
        [
            "skopeo",
            "inspect",
            "--no-tags",
        ...
        "rollback registry lookup",
    )
    try:
        registry_metadata = json.loads(inspect_output.decode("utf-8"))
    except (UnicodeError, json.JSONDecodeError) as exc:
        die(f"rollback registry lookup returned invalid JSON: {exc}")
    if not isinstance(registry_metadata, dict):
        die("rollback registry lookup must return an object")
    registry_name = registry_metadata.get("Name")
    if registry_name != repository:
        die("rollback registry repository identity does not match the intent")
    registry_digest = registry_metadata.get("Digest")
    if registry_digest != previous_digest:
        die("rollback registry digest does not match the requested previous digest")

    with tempfile.TemporaryDirectory(prefix="release-control-rollback-") as temporary:
        unbound_path = Path(temporary) / "provenance.unbound.json"
        with unbound_path.open("wb") as output:
            _run_external_command(
                [
                    ...
                    "--type",
                    COSIGN_PROVENANCE_TYPE,
                    *_cosign_workflow_claims(),
                    reference,
                ],
                "rollback provenance attestation verification",
                stdout=output,
            )
        try:
            unbound_statement = _load_verified_attestation_statement(
                unbound_path,
                COSIGN_PROVENANCE_TYPE,
                repository,
                previous_digest,
            )
            rollback_control_sha = _validate_rollback_provenance_value(
                unbound_statement["predicate"],
                intent,
                repository,
                previous_digest,
            )
        except ContractError as exc:
            die(f"rollback provenance attestation is invalid: {exc}")

        exact_path = Path(temporary) / "provenance.exact.json"
        with exact_path.open("wb") as output:
            _run_external_command(
                [
                    "cosign",
                    "verify-attestation",
                    "--type",
                    COSIGN_PROVENANCE_TYPE,
                    *_cosign_workflow_claims(rollback_control_sha),
                    reference,
                ],
                "rollback provenance attestation exact-workflow verification",
                stdout=output,
            )
        try:
            exact_statement = _load_verified_attestation_statement(
                exact_path,
                COSIGN_PROVENANCE_TYPE,
                repository,
                previous_digest,
            )
            exact_control_sha = _validate_rollback_provenance_value(
                exact_statement["predicate"],
                intent,
                repository,
                previous_digest,
            )
        except ContractError as exc:
            die(f"rollback provenance attestation exact-workflow proof is invalid: {exc}")
        if exact_control_sha != rollback_control_sha:
            die("rollback provenance workflow SHA changed between verification passes")
        if exact_statement["predicate"] != unbound_statement["predicate"]:
            die("rollback provenance predicate changed between verification passes")
    ...
    _run_external_command(
        ...
        "rollback artifact signature verification",
    )
    return {
        "status": "verified",
        "verification": "cosign-registry-and-provenance",
        "artifact": {
            "repository": repository,
            "digest": previous_digest,
            "reference": reference,
        },
        "registry": {"repository": registry_name, "digest": registry_digest},
        "workflow": {
            "identity": COSIGN_WORKFLOW_IDENTITY,
            "sha": rollback_control_sha,
            "issuer": COSIGN_OIDC_ISSUER,
            "trigger": COSIGN_WORKFLOW_TRIGGER,
            "name": COSIGN_WORKFLOW_NAME,
            "repository": COSIGN_WORKFLOW_REPOSITORY,
            "ref": COSIGN_WORKFLOW_REF,
        },
        "provenance": {"predicate_type": COSIGN_PROVENANCE_TYPE},
    }
...
def verify_release_bundles(
    ...
    """Cryptographically re-verify every downloaded release bundle and predicate."""

    require_directory(evidence_dir, "release evidence directory")
    control_sha = require_string(
        release_control_sha, GIT_SHA, "release-control SHA"
    )
    validate_release_bundle_files(intent, record, evidence_dir, control_sha)
    artifact = record["artifact"]
    repository = validate_artifact_repository(
        artifact["repository"], "release artifact repository"
    )
    if not repository.startswith(CANONICAL_ARTIFACT_PREFIX):
        die("release artifact repository is outside the canonical registry namespace")
    digest = require_nonzero_digest(artifact["digest"], "release artifact digest")
    reference = f"{repository}@{digest}"
    artifact_bundle = evidence_dir / EVIDENCE_FILE_NAMES["artifact_signature"]
    _run_external_command(
        [
            "cosign",
            "verify",
            "--bundle",
            str(artifact_bundle),
            *_cosign_workflow_claims(control_sha),
            reference,
        ],
    ...
    attestation_results: list[dict[str, str]] = []
    with tempfile.TemporaryDirectory(prefix="release-control-cosign-") as temporary:
        temporary_root = Path(temporary)
        for key, cosign_type, predicate_type, predicate_filename in COSIGN_ATTESTATION_SPECS:
            bundle_path = evidence_dir / EVIDENCE_FILE_NAMES[key]
            verified_path = temporary_root / f"{key}.json"
            with verified_path.open("wb") as output:
                _run_external_command(
                    [
                        "cosign",
                        "verify-attestation",
                    ...
                    f"{key} attestation verification",
                    stdout=output,
                )
            try:
                verify_attestation_payload(
                    verified_path,
                    evidence_dir / predicate_filename,
                    predicate_type,
                    repository,
                    digest,
                )
            except ContractError as exc:
                die(f"{key} attestation verification failed: {exc}")
            attestation_results.append(
                {"key": key, "predicate_type": predicate_type}
            )
    return {
        "status": "verified",
        "verification": "cosign-bundles-and-attestations",
        "artifact": {
            "repository": repository,
            "digest": digest,
            "reference": reference,
...
def verify_promotion_inputs(
    ...
    """Run every cryptographic admission check immediately before manifest emission."""

    bundles = verify_release_bundles(
        intent, record, evidence_dir, release_control_sha
    )
    rollback = verify_rollback_digest(intent)
    return {"status": "verified", "bundles": bundles, "rollback": rollback}
...
def main() -> int:
    ...
    promotion_verify_parser = sub.add_parser(
        "verify-promotion-inputs",
        help="cryptographically verify downloaded release and rollback inputs before emission",
    )
    promotion_verify_parser.add_argument("--intent", type=Path, required=True)
    promotion_verify_parser.add_argument("--release-record", type=Path, required=True)
    promotion_verify_parser.add_argument("--evidence-dir", type=Path, required=True)
    promotion_verify_parser.add_argument("--release-control-sha", required=True)
    ...
    try:
        ...
        elif args.command == "verify-promotion-inputs":
            intent = load_json(args.intent)
            require_canonical(args.intent, intent)
            record = load_json(args.release_record)
            require_canonical(args.release_record, record)
            result = verify_promotion_inputs(
                intent,
                record,
                args.evidence_dir,
                args.release_control_sha,
